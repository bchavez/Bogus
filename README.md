...
#### Using `IndexFaker`
`IndexFaker` is a per-`Faker<T>` index that automatically increments for each object generated by that `Faker<T>` instance. It is useful when a rule needs to know the current generated object position without managing a separate counter.
...
var employeeFaker = new Faker<Employee>()
    ...
    .RuleFor(e => e.Department, f => new[] { "Sales", "Support", "Engineering" }[f.IndexFaker % 3]);

var employees = employeeFaker.Generate(4);
var departments = employees.Select(e => e.Department).ToArray();
departments.Dump();
